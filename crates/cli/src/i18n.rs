...
impl Env {
    ...
    /// `GHOSTAI_PASSWORD=` is an operator clearing a variable in a wrapper
    ...
    /// Whether `GHOSTAI_DEBUG` asks for stack-shaped detail.
    #[must_use]
    pub fn debug(&self) -> bool {
        self.non_empty("GHOSTAI_DEBUG").is_some()
    }
}
...
impl Translations {
    ...
    pub fn t(&self, key: &str) -> String {
        self.translator.t(key, ghostai_i18n::args![])
    }

    /// One string with named arguments, built by [`ghostai_i18n::args!`].
    #[must_use]
    ...
    pub fn has(&self, key: &str) -> bool {
        self.translator.has(key, ghostai_i18n::args![])
    }
}

impl Default for Translations {
    fn default() -> Translations {
        Translations::new(ghostai_i18n::DEFAULT_LOCALE)
    }
}
...
/// structured detail a `GhostError` carries stays out of the sentence.
#[must_use]
pub fn describe_error(error: &GhostError) -> String {
    error.message.clone()
}
