...
mod agent_network {
    ...
    fn allowlist(allow: &[&str]) -> EnvironmentNetwork {
        EnvironmentNetwork {
    ...
    fn entries_without_the_allowlist_mode_are_refused_rather_than_ignored() {
        ...
        let mut network = allowlist(&["10.0.0.0/8"]);
    ...
    #[test]
    fn an_allowlist_with_no_entries_is_refused() {
        let refusal = assert_environment_network(&allowlist(&[]), "coder").unwrap_err();

        assert!(refusal.message.contains("reaches nothing"));
    }

    #[test]
    fn an_entry_the_grammar_refuses_is_named() {
        for entry in ["*.example.com", "example.com.", "10.0.0.0/33", "a b"] {
            let refusal = assert_environment_network(&allowlist(&[entry]), "coder").unwrap_err();

            assert!(
                refusal.message.contains("not a name"),
                "{entry}: {}",
                refusal.message
            );
            assert_eq!(refusal.details["entry"], entry);
            assert_eq!(refusal.details["agentId"], "coder");
        }
    }

    /// The packet filter cannot refuse these, so this is the only check there is.
    #[test]
    fn an_entry_in_a_range_nothing_may_reach_is_refused() {
        let refusal =
            assert_environment_network(&allowlist(&["169.254.169.254"]), "coder").unwrap_err();

        assert!(refusal.message.contains("which nothing may reach"));
    }

    /// The leading dot moved from refused to accepted when the three lists
    /// became one: it is now how a subdomain is spelled.
    #[test]
    fn every_shape_of_entry_is_accepted_in_one_list() {
        assert!(
            assert_environment_network(
                &allowlist(&["10.0.0.0/8", "93.184.216.34", "example.com", ".example.com",]),
                "a"
            )
            .is_ok()
        );
