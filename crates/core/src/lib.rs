...
//! processes here: those belong to `ghostai-security`, and a direct request or
//! spawn from this crate would bypass both guards. Filesystem access is allowed,
//! but only for paths GhostAI owns; nothing here resolves an agent-supplied path.
#![forbid(unsafe_code)]
...
pub use db::Database;
pub use errors::{ErrorKind, GhostError, Result};
