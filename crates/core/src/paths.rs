...
//! `darkwire-security`, which verifies through `realpath` and is the only thing
...
impl WirePaths {
    ...
    pub fn resolve(options: ResolveWirePaths) -> Result<WirePaths> {
...
pub fn shared_dir_for(paths: &WirePaths, workspace_id: &str) -> Result<PathBuf> {
...
pub fn extension_dir_for(paths: &WirePaths, id: &str) -> Result<PathBuf> {
...
pub fn extension_data_dir_for(paths: &WirePaths, id: &str) -> Result<PathBuf> {
...
/// reads as a sentence.
fn not_an_id(what: &str, id: &str) -> WireError {
    WireError::new(ErrorKind::InvalidInput, format!("Not {what} id: {id}")).with_detail("id", id)
}
