...
async fn main() {
    let result = async {
        if std::env::args().nth(1).as_deref() == Some("proxy") {
            ghostai_sandbox::proxy::serve(std::env::args().skip(2).collect()).await?;
            return Ok(());
        }
        if std::env::args().nth(1).as_deref() == Some("serve-env") {
            use ghostai_sandbox::service::{ServiceConfig, WorkspaceRegistration};
            let host = std::path::PathBuf::from(std::env::var("GHOSTAI_DATA_DIR")?);
            if !host.is_absolute() {
                return Err("GHOSTAI_DATA_DIR must be absolute".into());
            }
            ghostai_sandbox::service::serve(ServiceConfig {
        ...
        let path = std::env::args()
            .nth(1)
            .ok_or("Usage: ghostai-sandbox /path/to/service.json")?;
        let config = serde_json::from_slice(&std::fs::read(path)?)?;
        ghostai_sandbox::service::serve(config).await?;
