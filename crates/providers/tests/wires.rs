...
fn an_extension_fills_a_gap_but_cannot_replace_a_shipped_wire() {
    ...
    let provider = filled(darkwire_providers::WireAdapterOptions::new(spec)).unwrap();
    ...
    let ollama = darkwire_providers::find_builtin("ollama").unwrap().clone();
    let provider = kept(darkwire_providers::WireAdapterOptions::new(ollama)).unwrap();
