...
/// The base list a merged list was built from, and the merged list.
type MergedMemo = Option<(Arc<[ToolDefinition]>, Arc<[ToolDefinition]>)>;

/// A scope with a toolbox's own programs laid over it.
struct ToolboxOverlay {
    base: Arc<dyn ToolScope>,
    overlay: Vec<String>,
    scoped: Arc<dyn ToolScope>,
    cached: Mutex<MergedMemo>,
}

impl ToolboxOverlay {
    fn covers(&self, name: &str) -> bool {
        self.overlay.iter().any(|tool| tool == name)
    }
}

impl ToolScope for ToolboxOverlay {
    fn definitions(&self) -> Arc<[ToolDefinition]> {
        let underneath = self.base.definitions();
        let mut cached = self.cached.lock();
        // Memoised against the base's own list identity, which the registry
        // already keys on its revision — so an extension registering late still
        // shows up, and a turn that asks twice does not re-sort.
        if let Some((from, merged)) = &*cached
            && Arc::ptr_eq(from, &underneath)
        {
            return Arc::clone(merged);
        }
        let mut merged: Vec<ToolDefinition> = underneath
            .iter()
            .filter(|definition| !self.covers(&definition.name))
            .cloned()
            .chain(self.scoped.definitions().iter().cloned())
            .collect();
        merged.sort_by(|a, b| a.name.encode_utf16().cmp(b.name.encode_utf16()));
        let merged: Arc<[ToolDefinition]> = Arc::from(merged);
        *cached = Some((underneath, Arc::clone(&merged)));
        merged
    }
    ...
    fn permission_for(&self, name: &str) -> ToolPermission {
        if self.covers(name) {
            self.scoped.permission_for(name)
        } else {
            self.base.permission_for(name)
        }
    }
    ...
    fn execute<'a>(
        ...
        if self.covers(&call.name) {
            self.scoped.execute(call, ctx)
        } else {
            self.base.execute(call, ctx)
        }
    }
}
...
/// would then have to type. Each agent instead sees its own box's programs
/// under the names they actually have, and nothing is shared between agents.
///
...
/// the toolbox policy refuses the built-in names outright.
///
/// The overlay gets its own registry, so the abort race, the output cap and
/// the never-fails contract come from one implementation rather than two. It
/// gets the same permission map the base was built from, so a toolbox program
/// the agent switched off is filtered here exactly as a built-in would be. The
/// clock is handed in for the same reason: a toolbox program's `duration_ms`
/// must come off the same clock as a built-in's.
///
/// The timeout is deliberately *not* inherited. A toolbox program is a process
/// and the exec guard already gives it `plan.timeout_ms`; putting the agent's
/// `toolTimeoutMs` on top would add a second, tighter cap and start killing
/// container commands that run legitimately long.
pub fn with_toolbox_tools(
    base: Arc<dyn ToolScope>,
    tools: Vec<AnyTool>,
    permissions: ToolPermissions,
    ...
    let overlay: Vec<String> = tools
        .iter()
        .map(|tool| tool.definition().name.clone())
        .collect();
    let registry = Arc::new(ToolRegistry::with_options(ToolRegistryOptions {
        timeout_ms: 0,
        clock,
    }));
    registry.register_all(tools, ToolSource::Builtin)?;
    let scoped = registry.select(permissions);
    Ok(Arc::new(ToolboxOverlay {
        base,
        overlay,
        scoped,
        cached: Mutex::new(None),
