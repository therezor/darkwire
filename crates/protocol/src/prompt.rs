...
///
/// `index` is the whole of what memory contributes: one line per memory, its
/// key and its title. The contents are not offered, because they are not in
/// this section. The model reads the one it wants through the tool, which takes
/// the key and nothing else, so the folder is not offered either. `count` is
/// offered and unused by the default.
pub const MEMORY_PROMPT_PLACEHOLDERS: &[&str] = &["index", "count"];
...
///
/// It names `read` because a list of paths with no instruction to open
